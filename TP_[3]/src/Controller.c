...
int controller_addPassenger(LinkedList* pArrayListPassenger)
{
  ...
  if(pArrayListPassenger != NULL)
  {
	  ...
	  rtn = CargaDatos(aux,pArrayListPassenger);
	  if(rtn == 1)
	  {
...
int controller_sortPassenger(LinkedList* pArrayListPassenger)
{
	...
	if(pArrayListPassenger != NULL)
	{
		...
		RangoInt(&option, 1, 2, "1. Por nombre\n2. Por ID\nSu opcion: ", "Error. Reingrese su opcion (1-2):");

		if(option == 1)
		{
			RangoInt(&option, 0, 1, "0. Ascendiente\n1. Descendiente\nOpcion: ", "Error. Reingrese su opcion (0-1):");
			rtn = ll_sort(pArrayListPassenger, Passenger_CompararPorNombre, option);
		}
		else
		{
			RangoInt(&option, 0, 1, "0. Ascendiente\n1. Descendiente\nOpcion: ", "Error. Reingrese su opcion (0-1):");
