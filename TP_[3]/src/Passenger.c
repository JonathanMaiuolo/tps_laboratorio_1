...
int buscarPasajeroId(LinkedList* pArrayListPassenger)
{
	...
	SetNum(&id,"Ingresa el ID a buscar: ","ERROR. Reingrese devuelta el ID por favor: " );
...
int Passenger_CompararPorNombre(void* passenger, void* passenger2)
{
		char nombre[50];
		char nombre2[50];
...
int Passenger_CompararPorID(void* passenger, void* passenger2)
{
	Passenger* aux = passenger;
	Passenger* aux2 = passenger2;
...
int Passenger_printOne(Passenger* passenger)
{
	...
	if(passenger != NULL)
	{
		...
		Definir_tipoIntToStr(tipoPasajeroStr, &tipoPasajero);
